...
<tr>
    ...
    @endforeach
	<td>
        <button type="button" img-id="{{ $item->id }}" onclick="window.location = '{{ action("$controller@show" , array('id' => $item->id)) }}'" class="btn">Show</button>
        ...
        {!! Form::open(array('url' => action("$controller@destroy" , array('id' => $item->id)) , 'id' => 'drop_'.$item->id )) !!}
